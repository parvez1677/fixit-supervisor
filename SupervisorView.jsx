...
function SubmitTimesheetsPanel({sup}){
  ...
  const allW=ALL_WORKERS.filter(w=>sup.clients.includes(w.client));
...
function ReportTab({sup,inactive,medPending,advPending}){
  ...
  const myW=ALL_WORKERS.filter(w=>sup.clients.includes(w.client));
...
function AttendanceTab({sup,gradeOverrides={},inactive={emergencyLeave:0,annualLeave:0,absconder:0,refusal:0,resignation:0,training:0}}){
  ...
  const myWAll=ALL_WORKERS.filter(w=>sup.clients.includes(w.client));
    ...
    <>
      ...
      <Card s={{marginBottom:10}}>
        <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
          {["today","yesterday",...MONTHS].map(m=>(
            <button key={m}
              onClick={()=>{setViewMode(m);setSelDate(null);}} style={{fontSize:14,fontWeight:700,padding:"5px 12px",borderRadius:20,cursor:"pointer",border:"2px solid "+(viewMode===m?CB:LN),background:viewMode===m?CB:PNL,color:viewMode===m?"#fff":SFT,whiteSpace:"nowrap",textTransform:"capitalize"}}>{m}</button>
          ))}
        </div>
          ...
          return(
            <div key={w.id} style={{padding:"9px 0",borderTop:i?"1px solid "+LN:"none"}}>
              ...
              {expandedWorker===w.id&&(()=>{
                ...
                const moLabel=["today","yesterday"].includes(viewMode)?"Jun 2026":viewMode.replace(/ - Day \d+/,"");
...
function ComplaintsTab(){
  const [complaints,setComplaints]=useState([
    {id:1,worker:"Suresh Pillai",empCode:"EMP-1043",text:"Room AC not working since 3 days",status:"Open",date:"05 Jul",reply:""},
    {id:2,worker:"Ramesh Kumar",empCode:"EMP-1042",text:"Food quality complaint",status:"Resolved",date:"28 Jun",reply:"Raised with catering vendor. Menu revised from 01 Jul."},
  ]);
...
function MedicalTab(){
  ...
  const [items,setItems]=useState([
    {id:1,worker:"Bikash Thapa",empCode:"EMP-1045",type:"MWP",date:"10 Jun",supReason:"Medical certificate submitted",status:"pending",fileName:"medical_cert_bikash.pdf"},
    {id:2,worker:"Suresh Pillai",empCode:"EMP-1043",type:"MWOP",date:"12 Jun",supReason:"No paper - sick day",status:"pending",fileName:null},
  ]);
  const myW=ALL_WORKERS;
  const q=search.toLowerCase();
  const searchRes=showDrop&&q.length>0
    ?myW.filter(w=>w.name.toLowerCase().includes(q)||w.empCode.toLowerCase().includes(q)):[];
  const forward=(id)=>{setItems(items.map(m=>m.id===id?{...m,status:"forwarded"}:m));};
        ...
        <Card s={{marginBottom:10,border:"2px solid "+AMB}}>
          ...
          {items.filter(m=>m.status==="pending").map((m,i)=>(
            <div key={m.id} style={{padding:"10px 0",borderTop:i?"1px solid "+LN:"none"}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:8}}>
                <button style={{...PBtn,margin:0,flex:2,padding:"7px",fontSize:14,background:BLU}} onClick={()=>forward(m.id)}>Forward to Super Admin</button>
...
function IdleTab({goToOverride=()=>{}}){
  ...
  const [sent,setSent]=useState(false);
  const [items,setItems]=useState([
    {id:1,worker:"Suresh Pillai",empCode:"EMP-1043",date:"14 May",supReason:"No work assigned by client",status:"pending"},
  ]);
  const myW=ALL_WORKERS;
  const q=search.toLowerCase();
  const searchRes=showDrop&&q.length>0
        ...
        <Card s={{marginBottom:10,border:"2px solid "+AMB}}>
          ...
          {items.filter(m=>m.status==="pending").map((m,i)=>(
            <div key={m.id} style={{padding:"10px 0",borderTop:i?"1px solid "+LN:"none"}}>
              ...
              <div style={{fontSize:14,color:SFT,fontStyle:"italic",marginTop:2}}>"{m.supReason}"</div>
              <div style={{display:"flex",gap:8,marginTop:8}}>
                <button style={{...PBtn,margin:0,flex:2,padding:"7px",fontSize:14,background:YFG}} onClick={()=>forward(m.id)}>Approve - Send to Super Admin</button>
                <button style={{flex:1,padding:"7px",border:"1px solid "+RED,borderRadius:8,background:RBG,color:RED,cursor:"pointer",fontWeight:700,fontSize:14}}
                  onClick={()=>reject(m.id)}>Reject</button>
              </div>
            </div>
...
function SupervisorApp({sup}){
  ...
  return(
    <div style={{maxWidth:800,margin:"0 auto",minHeight:"100vh",background:BG}}>
      <div style={{background:"linear-gradient(135deg,"+DARK+","+CB+")",padding:"14px 16px",color:"#fff"}}>
      ...
      <div style={{padding:12}}>
        ...
        {tab==="actions"&&(
          <>
            ...
            {actionTab==="medical"  &&<MedicalTab/>}
            {actionTab==="idle"     &&<IdleTab goToOverride={()=>setActionTab("override")}/>}
            {actionTab==="advances" &&<AdvanceTab/>}
        ...
        {tab==="worker"&&(
          <>
            ...
            {workerTab==="complaints"  &&<ComplaintsTab/>}
          </>
        )}
...
export default function App(){
  ...
  if(loggedIn) return <SupervisorApp sup={loggedIn}/>;
  return(
    <div style={{maxWidth:340,margin:"60px auto",padding:24,background:PNL,borderRadius:16,boxShadow:"0 4px 20px rgba(27,58,138,.12)"}}>
      <div style={{textAlign:"center",marginBottom:24}}>
        ...
        <div style={{fontSize:15,color:SFT,marginTop:2}}>Supervisor Login - Enter PIN</div>
      </div>
