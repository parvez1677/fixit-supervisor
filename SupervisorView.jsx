...
function WorkerSalaryTab({sup}){
  ...
  const MO_NM=["Jan","Feb","Mar","Apr","May","Jun",
    "Jul","Aug","Sep","Oct","Nov","Dec"];
  const buildSalMonths=(worker)=>{
    if(!worker||!worker.stampDate)return [];
    const start=new Date(worker.stampDate);
    const now=new Date();
    const arr=[];
    let d=new Date(start.getFullYear(),start.getMonth(),1);
    while(d<=now){
      arr.push(MO_NM[d.getMonth()]+" "+d.getFullYear());
      d=new Date(d.getFullYear(),d.getMonth()+1,1);
    }
    return arr.reverse();
  };
  const SAL_DATA={
    "May 2026":{present:21,absent:1,idle:1,ot:5.0,uploaded:true,basic:1500,food:200,otPay:100,bonus:100,advance:200,fine:0,net:1700},
    "Apr 2026":{present:20,absent:2,idle:0,ot:0,uploaded:true,basic:1500,food:200,otPay:0,bonus:0,advance:0,fine:0,net:1700},
  };
  const salMonths=selWorker?buildSalMonths(selWorker):[];
  const activeMonth=selMonth||(salMonths[0]||"");
  const moData=(()=>{
    const base=SAL_DATA[activeMonth];
    if(base)return {lb:activeMonth,...base};
    return {lb:activeMonth,present:0,absent:0,idle:0,ot:0,uploaded:false};
  })();
  const MONTHS=activeMonth?[moData]:[];
  return(
    <>
      <Card s={{marginBottom:10}}>
      ...
      {selWorker&&(
        <Card s={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:10}}>
            <div>
              <div style={{fontSize:16,fontWeight:800,color:INK}}>{selWorker.name}</div>
              <div style={{fontSize:14,color:SFT}}>{selWorker.empCode} | Joined {selWorker.stampDate}</div>
            </div>
            <div style={{minWidth:150}}>
              <label style={{...lbl,marginTop:0}}>Select Month</label>
              <select style={{...inp,marginBottom:0}} value={activeMonth} onChange={e=>setSelMonth(e.target.value)}>
                {salMonths.map(m=><option key={m} value={m}>{m}</option>)}
              </select>
            </div>
          </div>
          <div style={{fontSize:13,color:SFT,marginTop:8,fontStyle:"italic"}}>
            Showing salary for {activeMonth}. {salMonths.length} month{salMonths.length!==1?"s":""} available since joining.
          </div>
        </Card>
      )}
